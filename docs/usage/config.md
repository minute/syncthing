...
\`\`\` {.sourceCode .xml}
 ...
 basic 1 0 0 0 random false 0 0 -1 false false false 25 .stfolder dynamic false 127.0.0.1:8384 k1dnz1Dd0rzTBjjFFh7CXPnrF12C49B1 default default default true true 21027 \[ff12::8384\]:21027 0 0 60 true 10 true true 60 30 10 0 0 LFWe2vn3 [https://data.syncthing.net/newdata](https://data.syncthing.net/newdata) false 1800 true 12 false 24 false 5 false 1 [https://upgrades.syncthing.net/meta.json](https://upgrades.syncthing.net/meta.json) false 10 0 auto default 24 false 25 false true 128 128 ~ 0 true
...
::: {.seealso} `introducer`{.interpreted-text role="ref"} :::
...
: Contains an address or host name to use when attempting to connect to this device. Entries other than `dynamic` must be prefixed with `tcp://` \(dual-stack\), `tcp4://` \(IPv4 only\) or `tcp6://` \(IPv6 only\). Note that IP addresses need not use tcp4/tcp6; these are optional. Accepted formats are:
...
: If given, this restricts connections to this device to only this network \(see `allowed-networks`{.interpreted-text role="ref"}\).

## GUI Element

\`\`\` {.sourceCode .xml}

 127.0.0.1:8384 l7jSbCqPD95JYZ0g8vi4ZLAMg3ulnN1b default
...
:   Authentication mode to use. If not present authentication mode
...
: BindDN for user authentication. Special %s variable shoild be used to pass username to LDAP.
...
\`\`\` {.sourceCode .xml}
 ...
 default default true true 21027 \[ff12::8384\]:21027 0 0 60 true 10 true true 60 30 10 0 [https://data.syncthing.net/newdata](https://data.syncthing.net/newdata) false 1800 true 12 24 false 5 false 1 [https://api.github.com/repos/syncthing/syncthing/releases?per\_page=30](https://api.github.com/repos/syncthing/syncthing/releases?per_page=30) false 10 ~
...
: The listen address for incoming sync connections. See `listen-addresses`{.interpreted-text role="ref"} for allowed syntax.
...
: The time to wait from startup to the first usage report being sent. Allows the system to stabilize before reporting statistics.
...
: If true, automatic upgrades include release candidates \(see `release-channels`{.interpreted-text role="ref"}\).
...
If you want to use Syncthing to backup your configuration files, it is recommended that the files you are backing up are in a `folder-sendonly`{.interpreted-text role="ref"} to prevent other devices from overwriting the per device configuration. The folder on the remote device\(s\) should not be used as configuration for the remote devices.

If you\'d like to sync your home folder in non-send only mode, you may add the folder that stores the configuration files to the `ignore list <ignoring-files>`{.interpreted-text role="ref"}. If you\'d also like to backup your configuration files, add another folder in send only mode for just the configuration folder.
