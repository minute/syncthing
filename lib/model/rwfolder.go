...
func (p *rwFolder) handleFile(file protocol.FileInfo, copyChan chan<- copyBlocksState, finisherChan chan<- *sharedPullerState) {
	...
	if err == nil {
		...
		// Copy the blocks, as we don't want to shuffle them on the file
...
func (p *rwFolder) pullerRoutine(in <-chan pullBlockState, out chan<- *sharedPullerState) {
	for state := range in {
		...
		var lastError error
		potentialDevices := p.model.Availability(p.folder, state.file.Name, state.block)
		for {
			// Select the least busy device to pull the block from. If we found no
			// feasible device at all, fail the block (and in the long run, the
			// file).
			selected, found := activity.leastBusy(potentialDevices)
			if !found {
				if lastError != nil {
...
func removeDevice(infos []AvailabilityInfo, info AvailabilityInfo) []AvailabilityInfo {
	for i := range infos {
		if infos[i] == info {
