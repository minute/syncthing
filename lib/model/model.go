...
func (m *Model) Availability(folder, file string, block protocol.BlockInfo) []AvailabilityInfo {
	...
	for _, device := range fs.Availability(file) {
		_, ok := m.conn[device]
		if ok {
			infos = append(infos, AvailabilityInfo{device, 0})
		}
	}

	for _, device := range devices {
		if m.deviceDownloads[device].Has(folder, file, block) {
			infos = append(infos, AvailabilityInfo{device, protocol.FlagFromTemporary})
		}
	}
